...
func PrivateEncrypt(data, key string) (string, error) {
	block, _ := pem.Decode([]byte(key))
	if block == nil {
		return "", errors.New("decode private key failed")
	}
	...
	return base64.URLEncoding.EncodeToString(signData), err
}

func PublicDecrypt(data, key string) ([]byte, error) {
	block, _ := pem.Decode([]byte(key))
	if block == nil {
	...
	if err != nil {
		return nil, errors.New("parse certifacte key failed")
	}
	...
	return publicDecrypt(pub, crypto.Hash(0), nil, bytes)
}
...
func PublicEncrypt(data, publicKey string) (string, error) {
	...
	pub := pubInterface.(*rsa.PublicKey)
	signData, err := rsa.EncryptPKCS1v15(rand.Reader, pub, []byte(data))
	return base64.StdEncoding.EncodeToString(signData), err
}

func PrivateDecrypt(data, privateKey string) ([]byte, error) {
